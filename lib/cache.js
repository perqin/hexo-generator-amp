...
function createCacheFile(inPath, hexoConfig, inPostPath , inDate , inEyeCatchImage , inTitleImageForAmp, inEyeCatchImageProperty , inXml){
  ...
  if( !fs.existsSync(getCachePath(hexoConfig)) ){
    cacheData = [
    ...
    for( var i=0; i<cacheData.length; i++){
      if( cacheData[i].path == inPostPath ){
        if( addObj.xml ){
          if( cacheData[i].date_amp != inDate ){
            cacheData[i] = assign(cacheData[i],addObj);
          }
        }else{
          if( cacheData[i].date_eyeCatch != inDate ){
            cacheData[i] = assign(cacheData[i],addObj);
          }
        }
        isAddedData = true;
...
module.exports.getCache = function(post, hexoConfig){
  ...
  if( existsOption(hexoConfig) && fs.existsSync(getCachePath(hexoConfig)) ){
    ...
    for( var i=0; i<read_cacheData.length; i++){
      if( read_cacheData[i].path == post.path ){
        if( read_cacheData[i].date_eyeCatch == String(post.updated._i) && read_cacheData[i].date_amp == String(post.updated._i) ){
          result = read_cacheData[i];
