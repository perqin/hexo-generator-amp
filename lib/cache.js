...
function createCacheFile(inPath, hexoConfig, inPostPath , inDate , inEyeCatchImage , inTitleImageForAmp, inEyeCatchImageProperty , inXml){
  ...
  if( !fs.existsSync(getCachePath(hexoConfig)) || hexoConfig.generator_amp.isCasheClear ){
    cacheData = {
    ...
    hexoConfig.generator_amp.isCasheClear = false; 
  }else{
...
module.exports.getCache = function(post, hexoConfig){
  var result = null;
  if( existsOption(hexoConfig) && fs.existsSync(getCachePath(hexoConfig)) && !hexoConfig.generator_amp.isCasheUpdate ){
    if(!read_cacheData){
