...
module.exports = function(locals){
  ...
  // copy template file
  //------------------------------------
  if(config.generator_amp && config.generator_amp.templateDir  && config.generator_amp.assetDistDir && config.generator_amp.logo.path && config.generator_amp.substituteTitleImage.path){
    ic.initCopy(config.generator_amp.templateDir, [ejsPath, cssPath, logoPath, substituteTitleImagePath]);
    var copyAssetsStatus  = ic.copyAssets(config.generator_amp.templateDir, config.generator_amp.assetDistDir, [config.generator_amp.logo.path, config.generator_amp.substituteTitleImage.path]);
    if(!copyAssetsStatus)return null;
  }else{
    console.log("\u001b[31m[hexo-generator-amp] (error) please setting option. ");
    return null;
  }
  ...
  //------------------------------------
  var ejsTemplateRelativePath = pathFn.basename(ejsPath);
  ...
  if(fs.existsSync(ampEjsTmplPath)){
    ...
    console.log("\u001b[31m[hexo-generator-amp] (error) not found template file. please check setting option.  path: "+ ampEjsTmplPath);
    return null;
  }
  ...
  // import CSS file
  //------------------------------------
  cssFilePath = pathFn.join(process.env.PWD , config.generator_amp.templateDir, pathFn.basename(cssPath) );
  ...
  if(fs.existsSync(cssFilePath)){
    ...
    console.log("\u001b[31m[hexo-generator-amp] (error) not found css file. please check setting option.  path: "+ cssFilePath);
    return null;
  }
